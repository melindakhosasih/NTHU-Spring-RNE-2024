    ...
    while(True):
        ...
        if path is not None and collision_count == 0:
            ...
            if args.simulator == "basic":
                ...
                next_v = 15 if end_dist > 20 else 0
                next_w, _ = controller.feedback(info)
                next_w = next_w if end_dist > 20 else 0
        ...
        if collision_count > 0:
            ...
            if args.simulator == "basic":
                command = ControlState("basic", -15, 0)
            elif args.simulator == "diff_drive":
                r = simulator.wu/2
                command = ControlState("diff_drive", np.rad2deg(-15 / r), np.rad2deg(-15 / r))
            elif args.simulator == "bicycle":
                command = ControlState("bicycle", (15 - simulator.state.v)*-0.5, 0)
            collision_count += 1
